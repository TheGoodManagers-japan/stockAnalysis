...
// Scrapes kabutan.jp news, extracts JP tickers, POSTs to dashboard API.
...
(() => {
    const CFG = {
      ...
      throttleMs: 400,
      maxPerCycle: 30,
      apiUrl: "http://localhost:3000/api/news/ingest",
      sendOnlyNew: true,
    ...
    async function sendPayload(items) {
      const sanitized = items.map(x => {
        // Send ALL JP tickers (not just the first one)
        const tickers = Array.isArray(x.tickers)
          ? x.tickers.filter(t => /^\d{4}\.T$/.test(t))
          : [];
