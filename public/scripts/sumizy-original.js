...
window.handleRefreshEventWithRG = (data, rgOverride = null) => {
  try {
    ...
    console.warn(
      `No refresh function found for type: ${refreshType}. Tried: ${functionsToTry.join(
        ", "
      )}`
    );
    return false;
    ...
    <div class="actions-row">
      <div class="reactions-list">
        ${emojis
          .map(
            (e) =>
              `<span class="emoji-option" data-id="${msgId}" data-emoji="${e}">${e}</span>`
          )
          .join("")}
      </div>
      ...
      ${
        canDelete
          ? `<div class="delete-action action-icon-button" data-id="${msgId}" title="Delete"><span class="action-icon">🗑️</span></div>`
          : ""
      }
    </div>`;
...
window.joinChannel = (userId, authToken, realtimeHash, channelOptions = {}) => {
  ...
  console.log(
    "joinChannel: Joining channel without specific RG - will detect dynamically"
  );
            ...
            try {
              ...
              if (data.action === "message" || data.action === "event") {
                  ...
                  if (!hasReceivedInitialHistory) {
                    ...
                    messagesToProcess = messagesToProcess.filter((msg) => {
                      if (optimisticallyRenderedMessages.has(msg.id)) {
                        console.log(
                          "Skipping already rendered message:",
                          msg.id
                        );
                ...
                if (relevantMessages.length > 0) {
                  ...
                  if (rg === null) {
                    console.warn(
                      "No visible RG found, skipping message injection"
                    );
                    continue;
                  }

                  console.log(
                    `Processing ${relevantMessages.length} messages for conversation ${chatId} in rg${rg} (action: ${data.action})`
                  );
                  ...
                  if (
                    data.action === "message" &&
                    relevantMessages.length > INITIAL_MESSAGE_LIMIT
                  ) {
                    ...
                    const olderMessages = relevantMessages.slice(
                      0,
                    ...
                    console.log(
                      `Showing ${messagesToInject.length} recent messages (${olderMessages.length} older messages available)`
                    );
                  } else {
                  ...
                  try {
                    await new Promise((resolve, reject) => {
                      ...
                      if (typeof window.injectMessages !== "function") {
                        reject(
                          new Error("window.injectMessages function not found")
                        );
                        return;
                      }
                    ...
                    console.log(
                      `Finished processing ${messagesToInject.length} messages in rg${rg}`
                    );
                    ...
                    if (typeof window[refreshFn] === "function") {
                      try {
                        window[refreshFn]();
                        console.log(
                          `Called ${refreshFn} after processing messages`
                        );
                      } catch (error) {
