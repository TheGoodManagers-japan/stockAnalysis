 ...
 ***********************************************/
window.scan = {
  async fetchStockAnalysis(sectorNumber) {
    try {
      // 1) Map sectorNumber => sector name
      const sectorList = [
        "Pharmaceuticals", // 1
      ...
      // Convert the numeric "sectorNumber" to the actual sector name
      const chosenSector = sectorList[sectorNumber - 1];
      if (!chosenSector) {
        console.error(`❌ Invalid sectorNumber: ${sectorNumber}`);
        return; // or throw an error if you prefer
      }

      // 2) Define all possible tickers
      const allTickers =  [
...
{code:"9532.T",sector:"Gas"}
      ...
      // 3) Filter tickers by the chosen sector
      const tickers = allTickers.filter((t) => t.sector === chosenSector);

      // If no tickers match, log a warning (optional)
      if (!tickers.length) {
        console.warn(`No tickers found for sector: ${chosenSector}`);
        return;
      }

      // 4) Loop through the filtered tickers
