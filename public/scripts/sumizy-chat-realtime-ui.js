...
window.showAITypingIndicator = function (rgNum) {
  ...
  const typingMessage = {
    ...
    isFile: false, _reactions: [], _translations: [],
  };
  ...
  typingIndicatorTimeout = setTimeout(() => window.hideAITypingIndicator(), 30000);
  window.aiTypingActive = true;
};

window.hideAITypingIndicator = function () {
  if (typingIndicatorTimeout) { clearTimeout(typingIndicatorTimeout); typingIndicatorTimeout = null; }
...
(function () {
  const isAIChatUrl = (href = location.href) => {
    ...
    return (path.includes("ai-chat") || q.has("ai-chat") || q.get("mode")==="ai" || q.get("chat")==="ai" || hash.includes("ai-chat"));
  };
...
window.handleRefreshEvent = (data) => {
  try {
    if (data.action !== 'event' || !data.payload || !data.payload.data) return false;
    let eventData;
    if (typeof data.payload.data === "string") { try { eventData = JSON.parse(data.payload.data); } catch { return false; } }
    else { eventData = data.payload.data; }
    if (!eventData.refresh) return false;

    const refreshType = eventData.refresh;
    const functionName = `bubble_fn_refresh${refreshType.charAt(0).toUpperCase()+refreshType.slice(1)}`;
  ...
  } catch (e) { console.error('Error handling refresh event:', e); return false; }
};

window.handleRefreshEventWithRG = (data, rgOverride = null) => {
  try {
    if (data.action !== 'event' || !data.payload || !data.payload.data) return false;
    let eventData; try { eventData = JSON.parse(data.payload.data); } catch { return false; }
    if (!eventData.refresh) return false;

    const refreshType = eventData.refresh;
    const capitalizedType = refreshType.charAt(0).toUpperCase() + refreshType.slice(1);
    const rg = rgOverride || (window.findVisibleRG ? window.findVisibleRG() : null);
    const tryFns = [];
    ...
    for (const fn of tryFns) {
      if (typeof window[fn] === 'function') {
        ...
        window[fn]({
          refreshType, dbo_id: data.payload.dbo_id, row_id: data.payload.row_id, originalData: eventData
        });
        return true;
      }
    }
    console.warn(`No refresh function found for type: ${refreshType}. Tried: ${tryFns.join(', ')}`);
    return false;
  } catch (e) { console.error('Error handling refresh event:', e); return false; }
};
...
function showFileViewer(url, name, isImage) {
  ...
  if (isImage) {
    ...
    const iconMap = { pdf:'📄', doc:'📝', docx:'📝', xls:'📊', xlsx:'📊', ppt:'📊', pptx:'📊', zip:'🗂️', rar:'🗂️', mp3:'🎵', wav:'🎵', mp4:'🎬', avi:'🎬', mov:'🎬', txt:'📃', code:'💻' };
    ...
    bodyContent = `
      <div class="file-viewer-icon-container" style="display:grid;gap:8px;place-items:center;padding:24px;color:#374151">
        ...
        <div class="file-viewer-filename" style="font-weight:600">${esc(name)}</div>
  ...
  ov.innerHTML = `
    <div class="file-viewer-modal" role="dialog" aria-label="${esc(name)}" tabindex="-1">
      ...
      <div class="file-viewer-footer">
        <div class="file-viewer-info">${fileSize ? `Size: ${fileSize}` : ''}</div>
  ...
  const modal = ov.querySelector('.file-viewer-modal'); modal.focus();
  ...
  if (isImage) {
    ...
    img.onerror = () => { loading.innerHTML = `<div class="file-viewer-icon">🚫</div><div>Failed to load image</div>`; };
  }

  const close = () => {
    ov.style.animation = 'fadeOut .2s ease-out';
    modal.style.animation = 'scaleOut .2s ease-out';
    setTimeout(() => { ov.remove(); document.removeEventListener('keydown', handleEscape); }, 200);
  };
  ...
  ov.querySelector('.file-viewer-download').addEventListener('click', async function() {
    ...
    try {
      ...
      this.classList.add('download-success'); this.innerHTML = `<svg class="download-icon" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg><span>Downloaded</span>`;
      setTimeout(()=>{ this.classList.remove('download-success'); this.innerHTML = `<svg class="download-icon" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="7 10 12 15 17 10"></polyline><line x1="12" y1="15" x2="12" y2="3"></line></svg><span>Download</span>`; }, 2000);
    } catch (err) {
...
document.addEventListener('click', e => {
  const fa = e.target.closest('.file-attachment'); if (!fa) return;
...
document.addEventListener('click', e => {
  ...
  menu.innerHTML = `
    <div class="actions-row" style="display:flex;align-items:center;gap:8px;background:#fff;border:1px solid #e5e7eb;padding:8px 10px;border-radius:10px;box-shadow:0 10px 30px rgba(0,0,0,.08)">
      <div class="reactions-list" style="display:flex;gap:6px;">
        ${emojis.map(e => `<span class="emoji-option" data-id="${esc(msgId)}" data-emoji="${e}" style="cursor:pointer">${e}</span>`).join('')}
      </div>
      <div class="action-divider" style="width:1px;height:18px;background:#e5e7eb;margin:0 4px;"></div>
      <div class="reply-action action-icon-button" data-id="${esc(msgId)}" title="Reply" style="cursor:pointer"><span class="action-icon">↩️</span></div>
      ${canDelete ? `<div class="delete-action action-icon-button" data-id="${esc(msgId)}" title="Delete" style="cursor:pointer"><span class="action-icon">🗑️</span></div>` : ''}
    </div>`;

  const r = e.target.getBoundingClientRect();
  Object.assign(menu.style, { position: 'absolute', right: `${innerWidth - r.right + 5}px`, top: `${r.bottom + 5}px`, zIndex: 9999 });
  ...
  menu.addEventListener('click', ev => {
    ...
    if (emo) {
      ...
      if (typeof window[fn] === 'function') window[fn]({ output1: emo.dataset.id, output2: emo.dataset.emoji });
      hideActionMenu(); return;
    }
    if (ev.target.closest('.reply-action')) {
      const fn = `bubble_fn_replyChat${rgNum}`;
      if (typeof window[fn] === 'function') window[fn]({ output1: buildReplyHtml(msgEl), output2: msgId });
      hideActionMenu(); return;
    }
    if (ev.target.closest('.delete-action')) {
      ...
      msgEl.remove(); hideActionMenu(); return;
    }
  });
});

document.addEventListener('click', e => {
  if (!e.target.closest('.message-actions-menu') && !e.target.classList.contains('message-actions-trigger')) hideActionMenu();
});
...
document.addEventListener('click', e => {
  ...
  if (typeof window[fn] === 'function') window[fn]({ output1: msg.dataset.id, output2: r.dataset.emoji });
});

/* ─────────── VISIBILITY QUEUE + OBSERVERS ─────────── */
/** Fallback finder if you don’t already define one elsewhere */
window.findVisibleRG ??= function () {
  const containers = Array.from(document.querySelectorAll('[id^="rg"] .chat-messages'));
  for (const el of containers) {
    const rg = el.closest('[id^="rg"]'); if (!rg) continue;
    const style = getComputedStyle(rg);
    const visible = rg.offsetParent !== null && style.visibility !== 'hidden' && style.display !== 'none' && el.offsetHeight > 0 && el.offsetWidth > 0;
...
window._chatVis               = window._chatVis || { observersReady: false, flushScheduled: false };

window.signalChatMaybeVisible = function () {
  requestAnimationFrame(() => _flushPendingIfVisible());
};
...
function _flushPendingIfVisible() {
  if (window._chatVis.flushScheduled) return;
  window._chatVis.flushScheduled = true;

  requestAnimationFrame(() => {
    window._chatVis.flushScheduled = false;
    const rg = window.findVisibleRG?.() ?? null; if (rg === null) return;
    const chatId = new URLSearchParams(location.search).get('chatid'); if (!chatId) return;

    const bucket = window._pendingChatInjections[chatId];
    if (!bucket || !bucket.length) return;
    const msgs = bucket.splice(0, bucket.length);
    ...
    try {
      if (typeof window.injectMessages === 'function') {
        ...
        if (typeof window[refreshFn] === 'function') { try { window[refreshFn](); } catch {} }
        if (window.aiTypingActive && !typingIndicatorElement) window.showAITypingIndicator(rg);
      } else {
        console.warn('injectMessages not found while flushing pending messages.');
      }
    } catch (e) {
...
function _setupChatVisibilityWatchers() {
  ...
  const attachIO = () => {
    const target = document.querySelector('.chat-messages'); if (!target) return;
    const io = new IntersectionObserver((entries) => {
      for (const entry of entries) if (entry.isIntersecting) _flushPendingIfVisible();
    }, { root: null, threshold: 0.01 });
    io.observe(target); window._chatVis.io = io;
  };
  attachIO();
...
window.enqueueOrInjectMessages = function (messages) {
  ...
  if (rg === null) { _enqueueForLater(chatId, messages); _setupChatVisibilityWatchers(); return; }
  ...
  try {
    if (typeof window.injectMessages === 'function') {
      window.injectMessages(rg, toInject, window.currentUserId);
      const refreshFn = `bubble_fn_refreshConversations${rg}`;
      if (typeof window[refreshFn] === 'function') { try { window[refreshFn](); } catch {} }
    }
  } catch (e) {
    console.error('enqueueOrInjectMessages → inject error:', e);
...
window.joinChannel = (userId, authToken, realtimeHash, channelOptions = {}) => {
  if (!userId || !authToken || !realtimeHash) { console.error('joinChannel: userId, authToken, and realtimeHash are required'); return; }

  let hasReceivedInitialHistory = false;

  try {
    const channelName = `sumizy/${userId}`;
    if (!window.xano) { console.error('joinChannel: Xano realtime not initialized. Make sure Xano is loaded.'); return; }

    window.xano.setAuthToken(authToken);
    window.xano.setRealtimeAuthToken(authToken);
    if (typeof window.xano.realtimeReconnect === 'function') window.xano.realtimeReconnect();
      ...
      const processMessageQueue = async () => {
        ...
        try {
          while (messageQueue.length > 0) {
            ...
            try {
              if (window.handleRefreshEvent && window.handleRefreshEvent(data)) continue;

              if (data.action === 'message' || data.action === 'event') {
                const urlParams = new URLSearchParams(window.location.search);
                const chatId = urlParams.get('chatid');
                if (!chatId) { console.warn('No chatId found in URL parameters'); continue; }
                ...
                if (data.action === 'message' && data.payload && Array.isArray(data.payload)) {
                  ...
                  if (typeof data.payload.data === 'string') {
                    try { messagesToProcess = JSON.parse(data.payload.data); }
                    catch (e) { console.error('Failed to parse event payload data:', e); continue; }
                  } else if (Array.isArray(data.payload.data)) {
                    messagesToProcess = data.payload.data;
                  }
                }

                if (!Array.isArray(messagesToProcess)) { console.warn('messagesToProcess is not an array:', messagesToProcess); continue; }

                const relevant = messagesToProcess.filter(message => String(message.conversation_id) === String(chatId));

                if (relevant.length > 0) {
                  const INITIAL_MESSAGE_LIMIT = 50;
                  let messagesToInject;

                  if (data.action === 'message' && relevant.length > INITIAL_MESSAGE_LIMIT) {
                    messagesToInject = relevant
                      .sort((a,b) => parseTime(a.created_at) - parseTime(b.created_at))
                      .slice(-INITIAL_MESSAGE_LIMIT);

                    const olderMessages = relevant.slice(0, -INITIAL_MESSAGE_LIMIT);
                  ...
                  try {
                    ...
                    if (rg !== null) {
                      ...
                      if (typeof window[refreshFn] === 'function') {
                        try { window[refreshFn](); } catch (error) { console.error(`Error calling ${refreshFn}:`, error); }
                      }
                    }
      ...
      channel.on((data) => {
        ...
        if (window.xanoRealtimeListeners) {
          window.xanoRealtimeListeners.map((x) => {
            if (x.data.channel == channelName || x.data.channel == null) x.data.message_received(data);
          });
        }
      });
    }
    ...
    window.currentChannel = {
      userId, channelName, channelKey, channel, authToken, realtimeHash, joinedAt: Date.now()
    };
    ...
    window.currentChannel = null; window.currentUserId = null;
    throw error;
  }
};

window.leaveChannel = (rg, userId) => {
  if (typeof rg !== 'number') { console.error('leaveChannel: rg must be a number'); return; }
  if (!userId) { console.error('leaveChannel: userId is required'); return; }
  ...
  try {
    ...
    if (window.xanoRealtime && window.xanoRealtime[channelKey]) {
      ...
      if (channel && typeof channel.disconnect === 'function') channel.disconnect();
    ...
    console.error('leaveChannel: Error leaving channel', error); throw error;
  }
};

window.getCurrentChannel = () => window.currentChannel || null;
window.isInChannel = (rg, userId) => window.currentChannel?.userId === userId && window.currentChannel?.rg === rg;

/* ─────────── LOAD OLDER MESSAGES ─────────── */
window.loadOlderMessages = (batchSize = 50) => {
  const urlParams = new URLSearchParams(window.location.search);
  const chatId = urlParams.get('chatid');
  if (!chatId) { console.error('loadOlderMessages: No chatid in URL'); return false; }
  if (!window.olderMessages || !window.olderMessages[chatId]) { console.log('loadOlderMessages: No older messages available'); return false; }

  const rg = window.findVisibleRG();
  if (rg === null) { console.error('loadOlderMessages: No visible RG found'); return false; }

  const olderMessages = window.olderMessages[chatId];
  if (!olderMessages.length) { console.log('loadOlderMessages: All older messages already loaded'); return false; }

  const messagesToLoad = olderMessages.splice(-batchSize, batchSize);
  if (!messagesToLoad.length) { console.log('loadOlderMessages: No more messages to load'); return false; }
...
window.getOlderMessagesCount = () => {
  const urlParams = new URLSearchParams(window.location.search);
  const chatId = urlParams.get('chatid');
  if (!chatId || !window.olderMessages || !window.olderMessages[chatId]) return 0;
...
window.sendMessage = (messageData) => {
  ...
  const channelInfo = window.currentChannel;
  if (!channelInfo) { console.error('sendMessage: No active channel. Join a channel first.'); return Promise.reject(new Error('No active channel')); }
  ...
  try {
    ...
    if (!thisChannel) { console.error('sendMessage: Channel object not found'); return Promise.reject(new Error('Channel object not found')); }
...
function extractLast10Messages() {
  ...
  return last10.map(el => ({ sender: el.dataset.username || "Unknown", message: el.dataset.message || "" }));
}
