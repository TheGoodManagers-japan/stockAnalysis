...
export function detectDipBounce(stock, data, cfg, U) {
  ...
  if (bounceStrengthATR <= 0.05) {
    ...
    // Early headroom snapshot (diagnostic only; orchestrator may gate on this)
    const resEarly = findResistancesAbove(data, px, stock) || [];
    const nearestResEarly = resEarly.length ? resEarly[0] : null;
    let headroomATR = null,
      headroomPct = null;
    if (nearestResEarly != null) {
