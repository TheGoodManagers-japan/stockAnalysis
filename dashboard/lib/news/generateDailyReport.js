...
/**
 * Generate a daily news report for a given date using Gemini.
 * Requires GEMINI_API_KEY in environment.
 * @param {string} dateStr - YYYY-MM-DD format
 * @returns {{ report, article_count, generated_at, report_date } | { empty: true }}
 */
export async function generateDailyReport(dateStr) {
  ...
  if (articles.length === 0) {
    return { empty: true, article_count: 0 };
  }
